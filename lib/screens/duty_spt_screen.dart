...
class _DutySPTScreenState extends State<DutySPTScreen> {
  ...
  // Role selection
  UserRole selectedRole = UserRole.maker; // 'conceptor/maker' or 'approval'
  ...
  Future<void> _fetchDuties() async {
    // If role not selected, do nothing
    ...
    try {
      ...
      if (getDutyListResponse.metadata.code == 200) {
        setState(() {
          // If maker, use 'duty'; if approver, use 'dutyApprove'
          duties = selectedRole == UserRole.maker
  ...
  void filterDuties() {
    setState(() {
      filteredDuties = duties.where((duty) {
        bool matchesRole = selectedRole == UserRole.maker
  ...
  bool _isConceptorMakerDuty(Duty duty) {
    // All possible statuses for the Maker
  ...
  bool _isApprovalDuty(Duty duty) {
    // All possible statuses for the Approver
  ...
  String _getStatusText(DutyStatus status, UserRole selectedRole) {
    if (selectedRole == UserRole.maker) {
      return status.makerDesc;
    } else {
                      ...
                      children: [
                        ...
                        if (selectedRole == UserRole.maker) ...[
                          ...
                          _buildStatusItem(
                            ...
                            label: DutyStatus.draft.makerDesc,
                            count: duties
                                .where(
                          ...
                          _buildStatusItem(
                            ...
                            label: DutyStatus.waiting.makerDesc,
                            count: duties
                                .where(
                          ...
                          _buildStatusItem(
                            ...
                            label: DutyStatus.returned.makerDesc,
                            count: duties
                                .where(
                                    (duty) => duty.status == DutyStatus.returned)
                                .length,
                            color: DutyStatus.returned.color,
                          ),
                          _buildStatusItem(
                            status: DutyStatus.approved,
                            label: DutyStatus.approved.makerDesc,
                            count: duties
                                .where(
                          ...
                          _buildStatusItem(
                            ...
                            label: DutyStatus.rejected.makerDesc,
                            count: duties
                                .where(
                                    (duty) => duty.status == DutyStatus.rejected)
                                .length,
                        ...
                        // If approver
                        if (selectedRole == UserRole.approver) ...[
                          const Text(
                            "AS AN APPROVAL",
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
  ...
  List<DropdownMenuItem<String>> _getStatusDropdownItems() {
    ...
    items.addAll(statuses.map((status) {
      String description = selectedRole == UserRole.maker
          ? status.makerDesc
          : status.approverDesc;
